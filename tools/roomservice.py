...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:MichaelAkhnoukh+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('MichaelAkhnoukh/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: MichaelAkhnoukh/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "MichaelAkhnoukh/%s" % repo_name })
...
print("Repository for %s not found in the MichaelAkhnoukh Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
